...
class MambaLayer(nn.Module):
    ...
    def mamba_forward(self, x):
        ...
        img_dims = x.shape[2:]
        x = x.reshape(bsz, channels, n_tokens).transpose(-1, -2)
        x = self.norm(x)
        x = self.mamba(x)
        x = x.transpose(-1, -2).reshape(bsz, channels, *img_dims)
        return x
...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        bsz, channels, _, _, _ = x.size()
        y = self.avg_pool(x).view(bsz, channels)
        y = self.fc(y).view(bsz, channels, 1, 1, 1)
        return x * y
    ...
    def __init__(
        ...
        for i in range(4):
            ...
            if i < 2:
                ...
                stage = nn.Sequential(*[MambaLayer(dim=dims[i]) for _ in range(depths[i])])
