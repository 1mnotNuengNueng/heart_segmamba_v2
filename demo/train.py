...
def _validate(model, loader, device) -> Dict[str, float]:
    ...
    return {"mean_dice": float(sum(all_scores) / max(len(all_scores), 1))}
...
def _save_history_csv(history: List[Dict[str, float]], path: Path) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["epoch", "train_loss", "val_mean_dice"])
        writer.writeheader()
...
def _save_training_plot(history: List[Dict[str, float]], path: Path) -> Optional[str]:
    ...
    ax2 = ax1.twinx()
    ax2.plot(epochs, val_dice, color="tab:blue", label="Val Mean Dice")
...
def train_model(
    ...
    # เพิ่ม Scheduler เพื่อลด LR ลงครึ่งหนึ่ง (factor=0.5) หาก Validation Dice ไม่เพิ่มขึ้นติดต่อกัน 5 Epochs
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        print(f"Epoch {epoch} | Train Loss: {epoch_record['train_loss']:.4f} | Val Dice: {epoch_record['val_mean_dice']:.4f}")
